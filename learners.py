...
class FreeRexSphereMomentum(FreeRexSphere):
    ...
    def dataset_initialize(self, dataset):
        super(FreeRexSphere, self).dataset_initialize(dataset)
        shape = dataset.shape
...
class MetaGradDiag(bm.Learner):
    ...
    def __init__(self, hyperparameters=None):
        if hyperparameters is None:
            hyperparameters = {'eta': 1.0}
        hyperparameters = {'eta': hyperparameters['eta']}
        super(MetaGradDiag, self).__init__('MetaGradDiag', hyperameters)
        
        self.eta_max = hyperparameters['eta']
    ...
    def dataset_initialization(self, dataset):
        ...
        self.parameter = np.zeros(shape)
        self.num_etas = np.ceil(0.5*np.log(self.T)/np.log(2.0))
        self.eta_values = [self.eta_max * np.pow(2, -x) * np.ones(shape) for x in xrange(self.num_etas)]
        ...
        self.prior = {eta: prior for eta,prior in enumerate(self.eta_values, priors)}

        self.alpha = 1.0/len(self.parameter.flatten())

        self.M = np.zeros(shape) + EPSILON

    def update(self, loss_info):
        gradient = loss_info['gradient']

        self.M += np.abs(gradient)**2

        ell_eta = lambda u, eta: eta*np.sum((u-self.parameter)*gradient) + eta**2 * np.sum((u-self.parameter)**2 * self.M)
        ...
        for eta in self.eta_values:
            sigma = 1.0/self.eta_max * 2*eta**2 * self.M
            old_prediction = self.eta_predictions[eta]
            new_prediction = old_prediction - 1.0/sigma * (eta*gradient + 2*eta**2 * self.M * (old_prediction - self.parameter))
